        ...
        <div class="content">
            ...
            <div class="loading-overlay" id="loadingOverlay">
                <div class="loading-spinner">
                    ...
                    <p class="mt-2">Loading car bookings...</p>
                </div>
            </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <div>
                                        <h4 class="header-title mb-0">
                                            ...
                                            Car Rental Bookings - Real-time Dashboard
                                        </h4>
                                        <p class="text-muted fs-14 mb-0">
                                            Live car rental data with automatic updates every 30 seconds
                                        </p>
                                    </div>
 ...
 <script>
            ...
            dataTable = $('#datatable-bookings').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        extend: 'csv',
                        className: 'btn btn-sm btn-secondary',
                        title: 'Car_Rental_Bookings_' + new Date().toISOString().split('T')[0]
                    },
                    {
                        extend: 'excel',
                        className: 'btn btn-sm btn-secondary',
                        title: 'Car Rental Bookings - ' + new Date().toLocaleDateString()
                    },
                    {
                        extend: 'pdf',
                        className: 'btn btn-sm btn-secondary',
                        title: 'Car Rental Bookings Report',
                ...
                language: {
                    emptyTable: "No car rental bookings found",
                    loadingRecords: "Loading bookings..."
                },
                columnDefs: [
                    { width: "150px", targets: 7 } // Route column
                ]
            });
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        updateLastUpdated();
                        showNotification('success', `Loaded ${response.count} car bookings`);
                    } else {
        ...
        function updateTable(bookings) {
            ...
            bookings.forEach(function(booking) {
                const passengersInfo = `${booking.adults}A - ${booking.children}C`;
                const statusBadge = `<span class="status-badge status-${booking.status_class.toLowerCase()}">${booking.status}</span>`;
                const routeBadge = `
                    <div class="route-badge">
                        <i class="bi bi-geo-alt-fill"></i> ${booking.pickup_location}
                    </div>
                    <div class="route-badge">
                        <i class="bi bi-flag-fill"></i> ${booking.dropoff_location}
                    </div>
                `;
                const carIcon = `<span class="car-icon"><i class="bi bi-car-front"></i></span>`;
                const actions = `
                ...
                dataTable.row.add([
                    ...
                    carIcon + booking.car_name,
                    booking.car_capacity + ' pax',
                    routeBadge,
                    booking.pickup_datetime,
                    booking.dropoff_datetime,
                    booking.duration,
                    passengersInfo,
                    booking.number_of_cars,
                    booking.total_price,
                    statusBadge,
                    actions
                ]);
